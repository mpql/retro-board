...
const changeColor = (color, ratio, darker) => {
  ...
  const decimal = rgb
    ...
    : color
        .replace(
          ...
          () =>
            `${Number.parseInt(arguments[1], 16)},${Number.parseInt(arguments[2], 16)},${Number.parseInt(arguments[3], 16)}`,
        )
        .split(/,/);
  ...
  return rgb
    ? `rgb${alpha !== null ? 'a' : ''}(${Math[darker ? 'max' : 'min'](
        Number.parseInt(decimal[0], 10) + difference,
        darker ? 0 : 255,
      )}, ${Math[darker ? 'max' : 'min'](
        Number.parseInt(decimal[1], 10) + difference,
        darker ? 0 : 255,
      )}, ${Math[darker ? 'max' : 'min'](
        Number.parseInt(decimal[2], 10) + difference,
        darker ? 0 : 255,
      )}${alpha !== null ? `, ${alpha}` : ''})`
    : // Return hex
      [
